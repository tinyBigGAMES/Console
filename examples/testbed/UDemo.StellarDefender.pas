...
type
  ...
  private
    ...
    FValue: Integer;
    FTag: Integer;
    FOwnsSprite: Boolean; // Added: flag to indicate if we own the sprite
  public
    ...
    property OwnsSprite: Boolean read FOwnsSprite write FOwnsSprite; // Added property
    procedure DecreaseHealth(Amount: Integer = 1);
    procedure IncreaseHealth(Amount: Integer = 1);
    procedure SetSpriteWithOwnership(ASprite: TAsciiSprite; AOwnSprite: Boolean = True); // New method
  end;
...
begin
  ...
  FValue := 0;
  FTag := 0;
  FOwnsSprite := False; // Default: don't own sprite
end;

destructor TEntity.Destroy;
begin
  // Free sprite if we own it
  if FOwnsSprite and Assigned(FSprite) then
    FSprite.Free;

  // Note: We don't free FAnimation as that's managed elsewhere
  inherited;
end;

procedure TEntity.SetSpriteWithOwnership(ASprite: TAsciiSprite; AOwnSprite: Boolean = True);
begin
  // Free old sprite if we own it
  if FOwnsSprite and Assigned(FSprite) then
    FSprite.Free;

  // Set new sprite and ownership flag
  FSprite := ASprite;
  FOwnsSprite := AOwnSprite;
end;
...
begin
  ...
  FBulletSprite.Free;
  FExplosionAnimation.Free;  // Just free the animation, the frames are managed internally
  FPowerupSprite.Free;
...
begin
  ...
  // Initialize background sprites array (will be filled in InitGame)
  for I := 0 to Length(FBackgroundSprites) - 1 do
...
begin
  ...
  FLevel := 1;
  FLives := 3;
  // Don't change FGameState here - leave it as is
  FGameTime := 0;
  FSpawnTimer := 0;
  FPowerupTimer := 0;
  ...
  TEntity(FEntities[FPlayerIndex]).SetSpriteWithOwnership(FPlayerSprite, False); // Don't own shared sprite
  ...
  for I := 0 to 30 do
  begin
    LStarEntity := TEntity.Create(etBackground, TConsole.RandomRange(0, ConsoleWidth),
                               TConsole.RandomRange(0, ConsoleHeight));

    // Create star sprite - this one is owned by the entity
    LStarSprite := TAsciiSprite.Create(1, 1);
    LStarSprite.SetChar(0, 0, '.', CSIFGWhite + CSIDim, CSIBGBlack);

    LStarEntity.SetSpriteWithOwnership(LStarSprite, True); // Entity owns this sprite
    LStarEntity.VelY := 0.1 + TConsole.RandomRange(0, 20) / 100;
    FEntities.Add(LStarEntity);
  end;
end;
...
begin
  ...
  begin
    for I := 0 to 2 do
    begin
      LOffsetX := TConsole.RandomRange(-2, 3);
      LOffsetY := TConsole.RandomRange(-2, 3);

      Explosion := TEntity.Create(etExplosion, X + LOffsetX, Y + LOffsetY);
      Explosion.Animation := FExplosionAnimation; // Use same shared animation
      Explosion.Animation.Reset;
...
begin
  ...
  Bullet.SetSpriteWithOwnership(FBulletSprite, False); // Don't own shared sprite
...
begin
  ...
  Enemy.SetSpriteWithOwnership(FEnemySprites[EnemyType mod 3], False); // Don't own shared sprite
  Enemy.Tag := EnemyType;
...
begin
  ...
  Asteroid.SetSpriteWithOwnership(FAsteroidSprites[Size], False); // Don't own shared sprite
  Asteroid.Tag := Size;
...
begin
  ...
  Powerup.SetSpriteWithOwnership(FPowerupSprite, False); // Don't own shared sprite
  Powerup.VelY := 0.2;
...
begin
  ...
  begin
    ...
    if Player.Sprite <> FPlayerSprite then
    begin
      // Player already has a custom sprite - just update it
      if Player.OwnsSprite then
      begin
        // Already has an owned sprite, just update its content
        Player.Sprite.LoadFromString(PLAYER_SPRITE_LEFT, PLAYER_COLOR, CSIBGBlack);
      end
      else
      begin
        // Create a new sprite
        LNewSprite := TAsciiSprite.Create(5, 3);
        LNewSprite.LoadFromString(PLAYER_SPRITE_LEFT, PLAYER_COLOR, CSIBGBlack);
        Player.SetSpriteWithOwnership(LNewSprite, True);
      end;
    end;
  end
  ...
  begin
    ...
    begin
      ...
      begin
        // Create a new sprite
        LNewSprite := TAsciiSprite.Create(5, 3);
        LNewSprite.LoadFromString(PLAYER_SPRITE_RIGHT, PLAYER_COLOR, CSIBGBlack);
        Player.SetSpriteWithOwnership(LNewSprite, True);
      end;
    end;
  end
  ...
  begin
    ...
    // If not already using the normal sprite, switch to it
    if Player.Sprite <> FPlayerSprite then
    begin
      Player.SetSpriteWithOwnership(FPlayerSprite, False);
    end;
  end;
...
var
  ...
  LConsoleWidth, LConsoleHeight: Integer;
begin
  for I := 0 to FEntities.Count - 1 do
  begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              // Respawn player
              TConsole.GetSize(@LConsoleWidth, @LConsoleHeight);

              // Create new player entity
              var LPlayerEntity := TEntity.Create(etPlayer, LConsoleWidth / 2, LConsoleHeight - 5);
              LPlayerEntity.SetSpriteWithOwnership(FPlayerSprite, False); // Don't own shared sprite
              LPlayerEntity.Health := 3;
              LPlayerEntity.CollisionShape := csRect;
              LPlayerEntity.FCollisionWidth := 5;
              LPlayerEntity.FCollisionHeight := 3;

              FPlayerIndex := FEntities.Add(LPlayerEntity);
            end;
          end;
        end
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              // Create new player entity with fixed variable name
              var LPlayerEntity := TEntity.Create(etPlayer, LConsoleWidth / 2, LConsoleHeight - 5);
              LPlayerEntity.SetSpriteWithOwnership(FPlayerSprite, False); // Don't own shared sprite
              LPlayerEntity.Health := 3;
              LPlayerEntity.CollisionShape := csRect;
        ...
        begin
          ...
          case LPowerup.Tag of
            0: begin // Extra points
                 Inc(FScore, 50);
                 FParticleSystem.Emit(LPowerup.X, LPowerup.Y, 10, CSIFGGreen, 0, -0.2, 0.5, 15);
               end;
            1: begin // Extra health
                 ...
                 FParticleSystem.Emit(LPowerup.X, LPowerup.Y, 10, CSIFGCyan, 0, -0.2, 0.5, 15);
               end;
            2: begin // Extra life
                 Inc(FLives);
                 FParticleSystem.Emit(LPowerup.X, LPowerup.Y, 10, CSIFGMagenta, 0, -0.2, 0.5, 15);
               end;
          end;

          LPowerup.Active := False;
        end;
      end;
    end;
  end;
...
var
  ...
  Entity: TEntity;
  ConsoleWidth, ConsoleHeight: Integer;
  Title, GameOverTitle, ScoreText, LevelText, ReplayText, QuitText: string;
  HealthChar: WideChar;
begin
  TConsole.GetSize(@ConsoleWidth, @ConsoleHeight);
    ...
    GAME_STATE_PLAYING, GAME_STATE_GAME_OVER:
      begin
        ...
        begin
          ...
          ScoreText := Format('Final Score: %d', [FScore]);
          ...
          LevelText := Format('Levels Completed: %d', [FLevel - 1]);
          ...
          ReplayText := 'Press [S] to Play Again';
...
var
  I: Integer;
  ConsoleWidth, ConsoleHeight: Integer;
  StatusText, HealthBar, FPSText: string;
  Player: TEntity;
begin
  TConsole.GetSize(@ConsoleWidth, @ConsoleHeight);
  ...
  FPSText := Format('FPS: %.1f', [FBuffer.ActualFPS]);
...
begin
  ...
  begin
    ...
    begin
      if FGameState = GAME_STATE_PLAYING then
      begin
        FGameState := GAME_STATE_TITLE;
        //InitGame;
      end
