...
begin
  while TConsole.IsKeyPressed(VK_ESC) do
  begin
  ...
  try
    repeat
      ...
      begin
        ...
        TConsole.Print(InfoString + StringOfChar(' ', 20)); // Pad with spaces
    ...
    begin
      ...
      TConsole.PrintLn('Error: %s', [E.Message]);
      TConsole.ResetTextFormat;
    end;
  end;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        TConsole.Print(NewState.Chars[I]);
      end;
    end;
    ...
    repeat
      ...
      begin
        ...
        TConsole.Print('Fractal Tree Animation - Angle: %.2f - Points: %d  ',
                      [BranchAngle * 180 / Pi, NewState.Count]);
...
var
  ...
  begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            TConsole.Print(' '); // Erase old position
          end;
        end;
      end;
    end;
    ...
    if LightningActive then
      TConsole.Print('Rain Effect Demo - LIGHTNING! - Drops: %d   ', [MAX_DROPS])
    else
      TConsole.Print('Rain Effect Demo - Drops: %d   ', [MAX_DROPS]);
  end;
...
var
  ...
  begin
    ...
    TConsole.SetCursorPos(2, ConsoleHeight - 1);
    TConsole.SetForegroundColor(CSIFGWhite);
    TConsole.Print('Fire Effect Demo - Time: %.1fs   ', [ElapsedMs / 1000.0]);
  end;
...
var
  ...
  begin
    ...
    TConsole.Print('Particle System Demo - Active: %3d/%3d', [ParticlesActive, MAX_PARTICLES]);
  end;
  ...
  try
    repeat
      ...
      begin
        ...
        TConsole.SetCursorPos(2, ConsoleHeight - 2);
        TConsole.SetForegroundColor(CSIFGWhite);
        TConsole.Print(InfoString + StringOfChar(' ', 20)); // Pad with spaces to overwrite
...
procedure Test_AdvancedAnimations;
const
  FRAMES = 30;
  WIDTH = 80;
  HEIGHT = 20;
...
begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            TConsole.Print(Buffer[IntY][IntX].Ch);
  ...
  TConsole.PrintLn('Advanced animation test completed successfully!');
  TConsole.PrintLn('Displayed %d frames of animated plasma effect', [FrameCount]);
  TConsole.PrintLn('Demonstrated: RGB colors, cursor positioning, and timing');
end;

// Advanced Console Effects Demo
procedure Test_AdvancedEffects;
const
  DURATION_MS = 5000;
  WAVE_WIDTH = 60;
  WAVE_HEIGHT = 15;
...
var
  ...
  begin
    ...
    TConsole.SetCursorPos(2, ConsoleHeight - 2);
    TConsole.SetForegroundColor(CSIFGWhite);
    TConsole.Print('Matrix Digital Rain - %d raindrops', [Length(RainDrops)]);
  end;
  ...
  begin
    ...
    TConsole.SetForegroundColor(CSIFGWhite);
    TConsole.Print('Sine Wave Animation - t: %.2f', [T]);
  end;
  ...
  begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            TConsole.Print(Fireworks[F][P].Ch);
          end;
        end;
      end;
    end;
    ...
    TConsole.SetCursorPos(2, ConsoleHeight - 2);
    TConsole.SetForegroundColor(CSIFGWhite);
    TConsole.Print('Fireworks Animation - %d active fireworks', [FireworkCount]);
  end;
  ...
  begin
    ...
    begin
      ...
      TConsole.Print(StarChar);
    end;

    // Draw info text
    TConsole.SetCursorPos(2, ConsoleHeight - 2);
    TConsole.SetForegroundColor(CSIFGWhite);
    TConsole.Print('Starfield Animation - time: %d', [TimeValue]);
  end;
  ...
  begin
    ...
    TConsole.SetForegroundColor(CSIFGWhite);
    TConsole.Print('TConsole Demo - Phase %d/%d', [DemoPhase + 1, TOTAL_PHASES]);
    ...
    if CompletedPhases < TOTAL_PHASES - 1 then
      TConsole.Print('Time: %.1fs (Next phase in %.1fs)',
                    [ElapsedMs / 1000.0,
                     PhaseTimeRemaining / 1000.0])
    else
      TConsole.Print('Time: %.1fs (Final phase)', [ElapsedMs / 1000.0]);

    // Show control info
    TConsole.SetCursorPos(2, ConsoleHeight - 1);
    TConsole.SetForegroundColor(CSIFGWhite);
    TConsole.Print('Press any key to exit demo...');
  end;
...
// Terminal-based Dashboard Demo with Flicker-Free Rendering
procedure Test_Dashboard;
const
...
type
  ...
  TScreenBuffer = array of array of record
    Ch: Char;
    FgColor: string;
    BgColor: string;
  end;
var
  ...
  begin
    ...
    for var Y := 0 to ConsoleHeight - 1 do
      for var X := 0 to ConsoleWidth - 1 do
      begin
        ScreenBuffer[Y][X].Ch := ' ';
        ScreenBuffer[Y][X].FgColor := CSIFGWhite;
        ScreenBuffer[Y][X].BgColor := CSIBGBlack;

        LastBuffer[Y][X].Ch := #0; // Invalid character to force initial draw
  ...
  // Render buffer to console (only changed characters)
  ...
  begin
    ...
    for Y := 0 to ConsoleHeight - 1 do
      for X := 0 to ConsoleWidth - 1 do
        if (ScreenBuffer[Y][X].Ch <> LastBuffer[Y][X].Ch) or
        ...
        begin
          ...
          // Draw character
          TConsole.Print(ScreenBuffer[Y][X].Ch);
  ...
  begin
    ...
    begin
      ...
      SetBufferChar(X + i, Y, BarChar, ColorCode);
    end;
  end;
  ...
  var
    ...
    ChartBuffer: array of array of record
      Ch: Char;
      Color: string;
    end;
  begin
    // Initialize chart buffer
    SetLength(ChartBuffer, Height, Width);

    for var cy := 0 to Height - 1 do
      for var cx := 0 to Width - 1 do
      begin
        ChartBuffer[cy][cx].Ch := ' ';
        ChartBuffer[cy][cx].Color := CSIFGBlack;
      end;

    // Draw axes
    for i := 0 to Width - 1 do
      ChartBuffer[Height - 1][i].Ch := '─';

    for i := 0 to Height - 1 do
      ChartBuffer[i][0].Ch := '│';

    ChartBuffer[Height - 1][0].Ch := '└';

    // Draw scale markers
    for i := 0 to 3 do
    begin
      var MarkerY := Height - 1 - (i * (Height - 1) div 3);
      if (MarkerY >= 0) and (MarkerY < Height) then
      begin
        ChartBuffer[MarkerY][0].Ch := '├';

        // Add scale display
        var ScaleValue := MinValue + (i * (MaxValue - MinValue) / 3);
        var ScaleText := Format('%.0f', [ScaleValue]);
        for var j := 0 to Min(Length(ScaleText) - 1, 4) do
          if (j + 1 < Width) and (MarkerY < Height) then
          begin
            // Fix: Ensure we don't go out of bounds of ScaleText
            if j+1 <= Length(ScaleText) then
              ChartBuffer[MarkerY][j + 1].Ch := ScaleText[j + 1]
            else
              Break;

            ChartBuffer[MarkerY][j + 1].Color := CSIFGBrightBlack;
          end;
      end;
    end;
    ...
    if MaxValue = MinValue then Scale := 0; // Fix division by zero
    ...
    begin
      ...
      if MaxValue > MinValue then
        CurrY := Height - 1 - Round((ValuePoint - MinValue) * Scale)
      else
        CurrY := Height - 1;

      // Ensure within bounds
      if CurrY < 0 then CurrY := 0;
      if CurrY >= Height then CurrY := Height - 1;

      // Draw point
      ChartBuffer[CurrY][CurrX].Ch := '●';
      ChartBuffer[CurrY][CurrX].Color := ColorCode;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if (LineX >= 0) and (LineX < Width) and
               (LineY >= 0) and (LineY < Height) and
               (ChartBuffer[LineY][LineX].Ch = ' ') then
            begin
              ChartBuffer[LineY][LineX].Ch := '·';
              ChartBuffer[LineY][LineX].Color := ColorCode;
            end;
          end;
        end;
      end;
    ...
    // Copy chart to screen buffer
    for var cy := 0 to Height - 1 do
      for var cx := 0 to Width - 1 do
        SetBufferChar(X + cx, Y + cy, ChartBuffer[cy][cx].Ch, ChartBuffer[cy][cx].Color);
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        TabFg := CSIFGBlack;
        TabBg := CSIBGWhite; // Fix: This was using CSIFGWhite instead of CSIBGWhite
      end
      else
      begin
        TabFg := CSIFGWhite;
      ...
      for var j := 0 to TabWidth - 1 do
      begin
        var ch: Char;
        if j < Length(TabText) then
          ch := TabText[j+1]
        else
          ch := ' ';

        SetBufferChar(TabX + j, Y, ch, TabFg, TabBg);
      end;
    ...
    for i := 0 to Width - 1 do
      SetBufferChar(X + i, Y + 1, '─', CSIFGWhite);
  end;

  // Clear buffer to prepare for new frame
  ...
  begin
    ...
    // Draw header
    for var X := 0 to ConsoleWidth - 1 do
      SetBufferChar(X, 0, ' ', CSIFGWhite, CSIBGBlue);

    SetBufferStr(1, 0, 'TConsole System Dashboard', CSIFGWhite, CSIBGBlue);

    // Draw current time
    var TimeStr := FormatDateTime('yyyy-mm-dd hh:nn:ss', Now);
    SetBufferStr(ConsoleWidth - Length(TimeStr) - 1, 0, TimeStr, CSIFGWhite, CSIBGBlue);
    ...
    case TabIndex of
      0: // Overview
        begin
          ...
          // CPU Box - Top Left
          var TitleStr := '┌─ ' + CPU.MetricName + ' ' + StringOfChar('─', BoxWidth - Length(CPU.MetricName) - 4) + '┐';
          SetBufferStr(MainX, MainY, TitleStr, CSIFGWhite);

          // CPU usage value
          var ValueStr := Format('%.1f%s', [CPU.CurrentValue, CPU.UnitName]);
          SetBufferStr(MainX + 2, MainY + 2, ValueStr, CPU.ColorCode);

          // CPU usage bar
          DrawBar(MainX + 2, MainY + 4, BoxWidth - 4, CPU.CurrentValue, CPU.MaxValue, CPU.ColorCode);

          // Memory Box - Top Right
          TitleStr := '┌─ ' + Memory.MetricName + ' ' + StringOfChar('─', BoxWidth - Length(Memory.MetricName) - 4) + '┐';
          SetBufferStr(MainX + BoxWidth + 1, MainY, TitleStr, CSIFGWhite);

          // Memory usage value
          ValueStr := Format('%.1f%s', [Memory.CurrentValue, Memory.UnitName]);
          SetBufferStr(MainX + BoxWidth + 3, MainY + 2, ValueStr, Memory.ColorCode);

          // Memory usage bar
          DrawBar(MainX + BoxWidth + 3, MainY + 4, BoxWidth - 4, Memory.CurrentValue, Memory.MaxValue, Memory.ColorCode);

          // Disk Box - Bottom Left
          TitleStr := '┌─ ' + Disk.MetricName + ' ' + StringOfChar('─', BoxWidth - Length(Disk.MetricName) - 4) + '┐';
          SetBufferStr(MainX, MainY + BoxHeight + 1, TitleStr, CSIFGWhite);

          // Disk usage value
          ValueStr := Format('%.1f%s', [Disk.CurrentValue, Disk.UnitName]);
          SetBufferStr(MainX + 2, MainY + BoxHeight + 3, ValueStr, Disk.ColorCode);

          // Disk usage bar
          DrawBar(MainX + 2, MainY + BoxHeight + 5, BoxWidth - 4, Disk.CurrentValue, Disk.MaxValue, Disk.ColorCode);

          // Network Box - Bottom Right
          TitleStr := '┌─ ' + Network.MetricName + ' ' + StringOfChar('─', BoxWidth - Length(Network.MetricName) - 4) + '┐';
          SetBufferStr(MainX + BoxWidth + 1, MainY + BoxHeight + 1, TitleStr, CSIFGWhite);

          // Network usage value
          ValueStr := Format('%.1f%s', [Network.CurrentValue, Network.UnitName]);
          SetBufferStr(MainX + BoxWidth + 3, MainY + BoxHeight + 3, ValueStr, Network.ColorCode);
      ...
      1: // CPU
        begin
          // Draw title
          SetBufferStr(MainX, MainY, 'CPU Usage History', CSIFGWhite);

          // Draw current value
          var ValueStr := Format('Current: %.1f%s', [CPU.CurrentValue, CPU.UnitName]);
          SetBufferStr(MainX, MainY + 2, ValueStr, CPU.ColorCode);

          // Draw min/max
          var StatsStr := Format('Min: %.1f%s  Max: %.1f%s', [CPU.MinValue, CPU.UnitName, CPU.MaxValue, CPU.UnitName]);
          SetBufferStr(MainX + 20, MainY + 2, StatsStr, CSIFGWhite);
          ...
          SetBufferStr(MainX, MainY + MainHeight - 3, 'CPU Info:', CSIFGWhite);
          SetBufferStr(MainX, MainY + MainHeight - 2, 'Intel Core i7-9700K @ 3.60GHz, 8 Cores, 8 Logical Processors', CSIFGWhite);
        end;

      2: // Memory
        begin
          // Draw title
          SetBufferStr(MainX, MainY, 'Memory Usage History', CSIFGWhite);

          // Draw current value
          var ValueStr := Format('Current: %.1f%s', [Memory.CurrentValue, Memory.UnitName]);
          SetBufferStr(MainX, MainY + 2, ValueStr, Memory.ColorCode);
          ...
          SetBufferStr(MainX, MainY + MainHeight - 3, 'Memory Info:', CSIFGWhite);
          SetBufferStr(MainX, MainY + MainHeight - 2, '16.0 GB DDR4-3200, 4 DIMMs, Dual-Channel', CSIFGWhite);
        end;

      3: // Disk
        begin
          // Draw title
          SetBufferStr(MainX, MainY, 'Disk I/O History', CSIFGWhite);

          // Draw current value
          var ValueStr := Format('Current: %.1f%s', [Disk.CurrentValue, Disk.UnitName]);
          SetBufferStr(MainX, MainY + 2, ValueStr, Disk.ColorCode);
          ...
          SetBufferStr(MainX, MainY + MainHeight - 3, 'Disk Info:', CSIFGWhite);
          SetBufferStr(MainX, MainY + MainHeight - 2, 'Samsung SSD 970 EVO 1TB, NVMe, 3500/2500 MB/s Read/Write', CSIFGWhite);
        end;

      4: // Network
        begin
          // Draw title
          SetBufferStr(MainX, MainY, 'Network Activity History', CSIFGWhite);

          // Draw current value
          var ValueStr := Format('Current: %.1f%s', [Network.CurrentValue, Network.UnitName]);
          SetBufferStr(MainX, MainY + 2, ValueStr, Network.ColorCode);
          ...
          SetBufferStr(MainX, MainY + MainHeight - 3, 'Network Info:', CSIFGWhite);
          SetBufferStr(MainX, MainY + MainHeight - 2, 'Intel Gigabit Network Connection, 1 Gbps, IP: 192.168.1.100', CSIFGWhite);
        end;
    end;
    ...
    begin
      var Ch: Char;
      if X < Length(HelpText) then
        Ch := HelpText[X+1]
      else
        Ch := ' ';

      SetBufferChar(X, ConsoleHeight - 1, Ch, CSIFGWhite, CSIBGBlack);
    end;
  ...
  begin
    ...
    begin
      ...
      // Fix: Need to handle the correct key codes
      case Ord(Key) of
        VK_ESC: // ESC
          Result := False; // Exit
...
procedure Test_AnimationPlayer;
const
...
var
  ...
  begin
    ...
    TConsole.PrintLn(Animations[CurrentAnimation].Title);
    TConsole.ResetTextFormat;
  ...
  begin
    ...
    begin
      ...
      TConsole.Print(Animations[CurrentAnimation].Frames[CurrentFrame].Data[i]);
    end;
...
begin
  ...
  begin
    TConsole.PrintLn('Console window too small for animation player.');
    TConsole.PrintLn('Please resize to at least 40x20 characters and try again.');
    Exit;
  end;
    ...
    for Y := 0 to ConsoleHeight - 1 do
      for X := 0 to ConsoleWidth - 1 do
        ...
        begin
          ...
          // Draw character
          TConsole.Print(ScreenBuffer[Y][X].Char);
...
begin
  ...
  begin
    TConsole.PrintLn('Console window too small for clock display demo.');
    TConsole.PrintLn('Please resize to at least %dx%d characters and try again.',
      [MIN_CONSOLE_WIDTH, MIN_CONSOLE_HEIGHT]);
    Exit;
  end;
...
begin
  ...
  TConsole.Print(CSIFGCyan+'         Version %s'+CRLF, [TConsole.GetVersion()]);
  ...
  Test_ColorTunnel();
  Test_FlowingText();
  Test_AdvancedAnimations();
  Test_AdvancedEffects();
